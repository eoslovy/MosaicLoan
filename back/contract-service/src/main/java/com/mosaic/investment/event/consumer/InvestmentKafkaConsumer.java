...
public class InvestmentKafkaConsumer {
    ...
    private static final String LOAN_EXECUTE_REQUEST = "loan.execute.requested";
    private final InvestmentService investmentService;
    private final ObjectMapper objectMapper;

    @KafkaListener(topics = INVEST_CREATE_COMPLETE, groupId = "investment.deposit.complete.consumer", concurrency = "3")
    public void executeInvestmentRequested(@Payload String payload) throws Exception {
        AccountTransactionPayload accountTransactionComplete = objectMapper.readValue(payload, AccountTransactionPayload.class);
        //TODO 웹소켓을 통한 성공 메세지 전달
        investmentService.completeInvestmentRequest(accountTransactionComplete);
        log.info("consumer 1, {}의 투자 계좌 생성이 정상적으로 처리되었습니다", accountTransactionComplete.accountId());
    }
    ...
    public void sendFailMessageInvestmentRequested(@Payload String payload) throws JsonProcessingException {
        AccountTransactionPayload accountTransactionFail = objectMapper.readValue(payload, AccountTransactionPayload.class);
    ...
    public void executeInvestmentToLoan(@Payload String payload) throws Exception {
        LoanCreateTransactionPayload accountTransaction = objectMapper.readValue(payload, LoanCreateTransactionPayload.class);
        //TODO 웹소켓을 통한 투자 시작 메세지 전달
        log.info("{}의 {}상품 대출 적합자 검색이 시작되었습니다", accountTransaction.accountId(), accountTransaction.interestRate());
