...
Default.args = {
  ...
  rawBarData: {
    "상환완료": Array.from({ length: 90 }, () => Math.floor(Math.random() * 150 + 50)),  // 50~200 사이 값
    "상환중": Array.from({ length: 90 }, () => Math.floor(Math.random() * 100 + 20)),  // 20~120 사이 값
    "투자중": Array.from({ length: 90 }, () => Math.floor(Math.random() * 80 + 10)),   // 10~90 사이 값
    "부실": Array.from({ length: 90 }, () => Math.floor(Math.random() * 50 + 5)),      // 5~55 사이 값
    "소유권 이전": Array.from({ length: 90 }, () => Math.floor(Math.random() * 40)),  // 0~40 사이 값
  },
  rawLineData: Array.from({ length: 90 }, (_, i) =>
    i < 30
      ? Math.floor(Math.random() * 400 + 500) // 500~900 (첫 30일)
      : i < 60
      ? Math.floor(Math.random() * 500 + 400) // 400~900 (중간 30일)
      : Math.floor(Math.random() * 600 + 300) // 300~900 (마지막 30일)
  ),
