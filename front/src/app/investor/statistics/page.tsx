          ...
          <div className={styles.chartRow}>
            <div className={styles.chartColumn}>
              <BarLineChart
                ...
                rawLineData={data.byAge.map((i) => typeof i.ratio === 'number' && !isNaN(i.ratio) ? i.ratio : 0)}
                barCategories={['거래건수']}
                lineLabel='비율 (%)'
              />
            </div>
            <div className={styles.chartColumn}>
              <BarChart
                labels={data.byAge.map((i) => i.group)}
                values={data.byAge.map((i) => (i.amount !== null && i.amount !== undefined && !isNaN(i.amount)) ? i.amount : 0)}
                title='거래 금액'
              />
            </div>
          </div>
          ...
          <div className={styles.chartRow}>
            <div className={styles.chartColumn}>
              <BarLineChart
                ...
                rawLineData={data.byFamilyStatus.map((i) => typeof i.ratio === 'number' && !isNaN(i.ratio) ? i.ratio : 0)}
                barCategories={['거래건수']}
                lineLabel='비율 (%)'
              />
            </div>
          ...
          <div className={styles.chartRow}>
            <div className={styles.chartColumn}>
              <BarLineChart
                ...
                rawLineData={data.byResidence.map((i) => typeof i.ratio === 'number' && !isNaN(i.ratio) ? i.ratio : 0)}
                barCategories={['거래건수']}
                lineLabel='비율 (%)'
              />
            </div>
      ...
      <div className={styles.treeMapContainer}>
        ...
        <IndustryTreemapChart
          data={data.byIndustry.map((item: RawIndustryRatio) => ({
            ...
            ratio: typeof item.ratio === 'number' && !isNaN(item.ratio) ? item.ratio : 0,
          }))}
        />
      </div>
    </main>
