  ...
  rest.post('/api/contract/investments/transactions/search', (req, res, ctx) => {
    ...
    try {
      ...
      if (investmentIds && Array.isArray(investmentIds) && investmentIds.length > 0) {
        ...
        console.log('필터링 전 트랜잭션 수:', filteredTransactions.length);
        console.log('현재 데이터의 투자 ID들:', [...new Set(filteredTransactions.map(t => t.investmentId))]);
        
        filteredTransactions = filteredTransactions.filter(transaction => 
          investmentIds.includes(transaction.investmentId)
        );
        
        console.log('투자 ID 필터링 후 트랜잭션 수:', filteredTransactions.length);
      }
      ...
      if (sort && Array.isArray(sort) && sort.length > 0) {
        ...
        filteredTransactions.sort((a, b) => {
          for (const sortItem of sort) {
            ...
            if (field === 'createdAt' || field === 'bondMaturity') {
