...
const Overview = () => {
  const { data, isError } = useQuery({ //, isLoading, 
  ...
  return (
    <section className={styles.wrapper}>
      <div className={styles.cardWrapper}>
        <BasicInfoCard icon="creditCard" value={`₩ ${Number(총투자금액).toLocaleString()}`} label="총 투자 금액" />
        <BasicInfoCard icon="trendingUp" value={`₩ ${Number(누적수익금).toLocaleString()}`} label="누적 수익금" />
        <BasicInfoCard icon="clock" value={`${평균수익률} %`} label="평균 수익률" />
        <BasicInfoCard icon="arrowUpRight" value={`${투자건수}건`} label="투자 건수" />
      </div>
    </section>
  );
};
